...
int main(int argc, const char * argv[]){
    ...
    DenteDiSega f;
    TH1F DenteDiSegaT("Trasformata", "Dente di sega - Trasformata", 100, 0., 1.);
    for(int i = 0; i<Times;i++) DenteDiSegaT.Fill(f.Evaluate(RandomNumber.Unif(0,1)));
    
    TH1F DenteDiSegaAR("AcceptReject", "Dente di sega - AcceptReject", 100, 0., 1.);
    ...
    alpha.Divide(2,1);
    alpha.cd(1);
    DenteDiSegaT.GetXaxis() -> SetTitle("Parameter value [AU]");
    DenteDiSegaT.GetYaxis() -> SetTitle("Function value [AU]");
    DenteDiSegaT.Scale(1./DenteDiSegaT.Integral("width"));
    DenteDiSegaT.Draw();
    DenteDiSegaT.Fit("pol1");
    
    alpha.cd(2);
    DenteDiSegaAR.GetXaxis() -> SetTitle("Parameter value [AU]");
    DenteDiSegaAR.GetYaxis() -> SetTitle("Function value [AU]");
    DenteDiSegaAR.Scale(1./DenteDiSegaAR.Integral("width"));
    DenteDiSegaAR.Draw();
    DenteDiSegaAR.Fit("pol1");
